...
# approved_chats  →  { "chat_id": { "title": ..., "type": ..., "added_at": ... } }
approved_chats: dict = load_json(APPROVED_CHATS_FILE, {})

# sessions → { "chat_id": { "mode": "study"|"quiz", "subject": ..., "exam": ..., "level": ..., "running": bool } }
...
# ─────────────────────────────────────────────
# GUARD: reject personal / non-approved chats
# ─────────────────────────────────────────────

async def guard(update: Update) -> bool:
    """
    Returns True  → allow processing
    Returns False → silently block
    """
    ...
    if chat.type == "private":
        if is_admin(user.id):
            return True          # admin always allowed in private
        # total silence for non-admins in private
        return False

    # Group / channel → must be approved
...
async def on_my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Triggered when bot is added / removed from a chat."""
    result = update.my_chat_member
    chat   = result.chat
    new_status = result.new_chat_member.status

    if new_status in ("member", "administrator"):
        # Auto-register the chat (pending admin approval)
        key = str(chat.id)
...
# ─────────────────────────────────────────────
# state keys in context.user_data (private admin DM) or sessions
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Count selection → confirm & launch ──
...
"""
        ...
        for q in questions[:this_batch]:
            ...
            try:
                ...
                save_sessions()
                await asyncio.sleep(32)  # wait for poll to expire + small buffer
            except Exception as e:
...
# ─────────────────────────────────────────────
# FALLBACK — ignore non-admin messages
# ─────────────────────────────────────────────

async def fallback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await guard(update):
        return
    # Do nothing for non-command messages from non-admins
...
def main():
    ...
    if not ADMIN_IDS:
        raise ValueError("ADMIN_IDS environment variable not set! (comma-separated Telegram user IDs)")
    ...
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL | filters.ChatType.GROUPS | filters.ChatType.PRIVATE, fallback_handler))

    # Track bot being added/removed
